...
// SA_FATAL_FIXED_B_450: include SA so $sql is defined for super-admin (was B-450/B-504)
...
if (!empty($appointRegisterId) && !empty($orgId)) {
    ...
    $doctorCondition  = !empty($allowedDoctorIds) 
        ...
        : "AND 0"; // no allowed doctors → no results
...
<script>
    ...
    <div class="main-content">
        <section class="section">
            ...
            <div class="card">
                <div class="card-header">
                    <h4>Patient Test History Reports List</h4>
                </div>
...
function getOPTestReportsData() {
    $.ajax({
        url: "ajax/optestreports/gettesthsitoryreports.php",
        type: "GET",
        success: function (response) {
...
function getAppointIDFromURL() {
  ...
  return urlParams.get('appointRegisterId');
   ...
   function savePatientTestHistory(isAppointDataPresent) {
    ...
    if (!appoint_unicode || !appoint_register_id || !org_id || !doctor || !specialization) {
    ...
    $.ajax({
        url: 'ajax/optestreports/savereports.php',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
