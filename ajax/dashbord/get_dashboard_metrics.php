...
$SessionUserId = $_SESSION['security_id'] ?? '';       // default
if (!empty($_GET['security_id'])) {              // allow override
    $SessionUserId = intval($_GET['security_id']);
}
...
$avgWaitingTime = mysqli_fetch_array(mysqli_query(
    ...
    "SELECT 
     ...
     FROM appointment_online AS a
     JOIN doctor_patient_duration AS dpd
...
$sql = " SELECT COUNT(DISTINCT d.doc_id) AS active_doctors FROM doctors AS d LEFT JOIN doctors_time_slot 
AS ts ON ts.doctorName_registrationNumber = d.doc_id AND ts.available_date = CURDATE() AND 
ts.status='1' LEFT JOIN multi_doctortimeslots AS mts ON mts.doctorName_registrationNumber = d.doc_id 
AND CURDATE() BETWEEN mts.from_date AND mts.to_date AND FIND_IN_SET(DAYOFWEEK(CURDATE()), mts.selectedDays)
 AND mts.status='1' WHERE d.status='1' AND d.security_id='$SessionUserId' AND 
 (ts.doctors_time_id IS NOT NULL OR mts.multi_id IS NOT NULL) "; 
 $doctorsOnDuty = mysqli_fetch_assoc(mysqli_query($conn, $sql))['active_doctors'];


 // --- Doctor info ---
$doctorRow = mysqli_fetch_assoc(mysqli_query(
    $conn,
    "SELECT 
         d.doctor_name,
         ds.specialtisname,
     ...
     WHERE d.security_id = '$SessionUserId'"
));
