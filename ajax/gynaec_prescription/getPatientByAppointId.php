...
$regId = mysqli_real_escape_string($conn, $_POST['appoint_register_id'] ?? '');
...
if ($SessionUserId == "1" && $SessionRoleId == "1") {
    $q = mysqli_query($conn, "SELECT patient_name, mobile_number, appoint_register_id, appoint_unicode, appoint_id, age, gender, dob
        FROM appointment_online WHERE appoint_status='1' AND appoint_register_id='$regId' $docScope
        ORDER BY appoint_id DESC LIMIT 1") or die(mysqli_error($conn));
} else {
    $q = mysqli_query($conn, "SELECT patient_name, mobile_number, appoint_register_id, appoint_unicode, appoint_id, age, gender, dob
        FROM appointment_online WHERE appoint_status='1' AND org_id='$SessionOrgId' AND appoint_register_id='$regId' $docScope
        ORDER BY appoint_id DESC LIMIT 1") or die(mysqli_error($conn));
}
