...
$getDoctorTime = mysqli_query($conn, "SELECT m.doctors_time_id, mr.starting_Time, mr.ending_Time FROM doctors_time_slot m, doctors_time_slot2 mr WHERE m.status='1' AND m.doctors_time_id=mr.doctors_time_id AND m.doctorName_registrationNumber='$doctorName_registrationNumber' AND m.available_date='$appoint_date' AND mr.org_id='$organizations' AND m.org_id=mr.org_id ORDER BY m.doctors_time_id DESC") or die(mysqli_error($conn));
